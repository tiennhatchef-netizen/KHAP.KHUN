...
function signToken(user) {
  return jwt.sign(
    { id: user.id, username: user.username, name: user.name, role: user.role, is_superadmin: user.is_superadmin || false },
    SECRET,
    { expiresIn: '30d' }
  );
}
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    res.json({
      token,
      user: { id: user.id, username: user.username, name: user.name, role: user.role, is_superadmin: user.is_superadmin || false }
    });
  } catch (e) {
